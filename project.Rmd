...
# X_train = training metabolite matrix (samples x metabolites)
# y_train = training class labels (Male/Female)

# 1. Compute raw p-values using Welch t-test for each metabolite
pvals <- apply(X_train, 2, function(x) t.test(x ~ y_train)$p.value)

# 2. FDR correction (Benjamini-Hochberg)
pvals_fdr <- p.adjust(pvals, method = "fdr")

# 3. Compute mean fold change ( Male / Female )
